...
//    }
     ...
     * <p>Adds all the elements of the given arrays into a new array.</p>
     * <p>The new array contains all of the element of <code>array1</code> followed
     * by all of the elements <code>array2 .. arrayN</code>. When an array is returned, it is always
     * a new array.</p>
     * @param array1  the first array whose elements are added to the new array.
     * @param arrays  arrays whose elements are added to the new array.
     * @return The new byte[] array.
     * @since 2.1
     */
    public static byte[] addAll(byte[] array1, byte[]... arrays) {
        if (arrays == null) {
            return clone(array1);
        }
        int total = array1.length;
        for (int i = 0; i < arrays.length; i++) {
			total += arrays[i].length;
		}
        byte[] joinedArray = new byte[total];
        System.arraycopy(array1, 0, joinedArray, 0, array1.length);
        int current = array1.length;
        for (int i = 0; i < arrays.length; i++) {
			System.arraycopy(arrays[i], 0, joinedArray, current, arrays[i].length);
			current += arrays[i].length;
		}
        return joinedArray;
    }
	...
	public static byte[] singleInt2Byte(int src) {
		byte[] dst = new byte[4];
		int j = 0;
		dst[j++] = (byte) ((src >>> 0) & 0xff);
		dst[j++] = (byte) ((src >>> 8) & 0xff);
		dst[j++] = (byte) ((src >>> 16) & 0xff);
		dst[j++] = (byte) ((src >>> 24) & 0xff);
		return dst;
	}
	
	public static byte[] singleLong2Byte(long l) {
		byte[] dst = new byte[8];
		int j = 0;
	...
	public static byte[] long2byte(long[] src) {
		...
		for (int i = 0; i < srcLength; i++) {
			...
			dst[j++] = (byte) ((x >>> 16) & 0xff);
			dst[j++] = (byte) ((x >>> 24) & 0xff);
			dst[j++] = (byte) ((x >>> 32) & 0xff);
			dst[j++] = (byte) ((x >>> 40) & 0xff);
			dst[j++] = (byte) ((x >>> 48) & 0xff);
			dst[j++] = (byte) ((x >>> 56) & 0xff);
		}
		return dst;
	}
	...
	public static int byte2SingleInt(byte[] src) {
		if (src == null || src.length != 4)
			throw new IllegalArgumentException("Need 4 bytes to convert to int");
	...
	public static byte[] double2byte(double[] data) {
		...
		System.arraycopy(double2byte(data[i]), 0, byts, i * 8, 8);
		return byts;
	}
	
	public static byte[] double2byte(double data) {
//		return ByteBuffer.allocate(8).putDouble(data).array();
		return singleLong2Byte(Double.doubleToRawLongBits(data));
	}
		
	public static double byte2SingleDouble(byte[] data) {
		if (data == null || data.length != 8) return 0x0;
		// ---------- simple:
		return Double.longBitsToDouble(toLong(data));
		}
		
	public static double[] byte2double(byte[] data) {
		if (data == null)
			return null;
		// ----------
		if (data.length % 8 != 0)
		...
		for (int i = 0; i < dbls.length; i++) {
			dbls[i] = byte2SingleDouble(new byte[] { data[(i * 8)],
					data[(i * 8) + 1], data[(i * 8) + 2], data[(i * 8) + 3],
					data[(i * 8) + 4], data[(i * 8) + 5], data[(i * 8) + 6],
					data[(i * 8) + 7], });
		}
		return dbls;
	}
    ...
    public static int identityHashCode(Object a[]) {
        if (a == null)
            return 0;
