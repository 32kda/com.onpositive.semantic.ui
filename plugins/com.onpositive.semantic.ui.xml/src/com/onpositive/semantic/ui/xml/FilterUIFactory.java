...
public class FilterUIFactory extends UIElementHandler {
	...
	public Object handleElement(Element element, Object parentContext,
			Context context) {
		final FilterControl handleElement = (FilterControl) super
				.handleElement(element, parentContext, context);
		...
		if ((attribute != null) && (attribute.length() > 0)) {
			context.addInitializer(new IInitializer() {
				...
				public void init(Context context) {
					handleElement
							.setSelector((AbstractEnumeratedValueSelector<?>) context
									.getObject(attribute));
				}
