...
public class CommandHandler extends GeneralElementHandler implements IElementHandler {
	...
	public Object handleElement(Element element, Object parentContext, Context context)
	{
		if (parentContext instanceof AbstractUIElement<?>) {
			...
			final EditorBindingController editorBindingController = new EditorBindingController( keyHandler, attribute )
			{
				boolean inited;

				public void hierarchyChanged(IUIElement<?> element) {
					if (!this.inited) {
						final IAllowsRegisterCommands dd = (IAllowsRegisterCommands) element.getService(IAllowsRegisterCommands.class);
						if (dd != null) {
			...
			editorBindingController.hierarchyChanged(el);
