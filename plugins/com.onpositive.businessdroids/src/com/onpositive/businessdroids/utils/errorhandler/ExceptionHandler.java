...
package com.onpositive.businessdroids.utils.errorhandler;
...
final class ExceptionHandler implements Thread.UncaughtExceptionHandler {
	...
	private ExceptionHandler(Context context) {
		...
		try {
			...
			this.versionName = mPackInfo.versionName;
			this.versionCode = mPackInfo.versionCode;
		} catch (PackageManager.NameNotFoundException e) {
			// ignore
		}
		this.previousHandler = Thread.getDefaultUncaughtExceptionHandler();
		this.stacktraceDir = String.format("/Android/data/%s/files/",
	...
	public void uncaughtException(Thread thread, Throwable exception) {
		...
		if (Environment.MEDIA_MOUNTED.equals(state)) {
			...
			reportBuilder
					.append("\n\n\n")
					.append(this.formatter.format(dumpDate))
					.append("\n")
					.append(String.format("Version: %s (%d)\n",
							this.versionName, this.versionCode))
					.append(thread.toString()).append("\n");
			this.processThrowable(exception, reportBuilder);

			File sd = Environment.getExternalStorageDirectory();
			File stacktrace = new File(sd.getPath() + this.stacktraceDir,
					String.format("stacktrace-%s.txt",
							this.fileFormatter.format(dumpDate)));
		...
		if (this.previousHandler != null) {
			this.previousHandler.uncaughtException(thread, exception);
		}
	}
	...
	private void processThrowable(Throwable exception, StringBuilder builder) {
		...
		this.processThrowable(exception.getCause(), builder);
	}
}