 ...
 *******************************************************************************/
package com.onpositive.internal.ui.text.spelling.engine;
...
public class DefaultSpellChecker implements ISpellChecker {
	...
	public void execute(final ISpellEventListener listener,
		...
		final boolean ignoreURLS = fPreferences
				.getBoolean(PreferenceConstants.SPELLING_IGNORE_URLS);
		final boolean ignoreNonLetters = fPreferences
				.getBoolean(PreferenceConstants.SPELLING_IGNORE_NON_LETTERS);
		final boolean ignoreSingleLetters = fPreferences
				.getBoolean(PreferenceConstants.SPELLING_IGNORE_SINGLE_LETTERS);
		final int problemsThreshold = PreferenceConstants.getPreferenceStore()
				.getInt(PreferenceConstants.SPELLING_PROBLEMS_THRESHOLD);
	...
	public Set getCompletionProposals(final String word, boolean sentence) {
		// synchronizing might not be needed here since getProposals is
		// a read-only access and only called in the same thread as
		// the modifing methods add/removeDictionary (?)
		Set copy;
		...
		for (final Iterator iterator = copy.iterator(); iterator.hasNext();) {
			...
			dictionary = (ISpellDictionary) iterator.next();
			if (dictionary instanceof AbstractSpellDictionary) {
				final AbstractSpellDictionary sp = (AbstractSpellDictionary) dictionary;
